...
dataset_path1 = config.get("train", "dataset_path1")
dataset_path2 = config.get("train", "dataset_path2")
dataset_path3 = config.get("train", "dataset_path3")
...
dataset = FaceEmbed([dataset_path1, dataset_path2, dataset_path3], same_prob=0.5)
...
for epoch in range(0, max_epoch):
	...
	for iteration, data in enumerate(dataloader):
		step = len(dataloader) * epoch + iteration
		...
		loss_fake = 0
		for di in fake_D:
			loss_fake += torch.sum(hinge_loss(di[0], False).mean(
				dim=[1, 2, 3]) * diff_person) / (diff_person.sum() + 1e-4)

		true_D = D(Xs)
		loss_true = 0
		for di in true_D:
			loss_true += torch.sum(hinge_loss(di[0], True).mean(
				dim=[1, 2, 3]) * diff_person) / (diff_person.sum() + 1e-4)
		...
		if step % 2000 == 0:
			torch.save(G.state_dict(), '%s/G_latest_%s.pth' % (model_save_path, str(step)))
			torch.save(D.state_dict(), '%s/D_latest_%s.pth' % (model_save_path, str(step)))
